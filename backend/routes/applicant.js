...
router.post("/saveEasyApply", async (req, res, next) => {
  ...
  var lessArr = [0];
  lessArr[0] += 1;
  const newLessData = new LessChart({
    dateApplied: todayDate,
    jobId : req.body.jobId,
    jobTitle : req.body.jobTitle,
    month: month,
...
// new Chart less application
LessChart.findOne({ jobTitle: req.body.jobTitle}, function (err, chartRow) {
    console.log(err);
    if (chartRow) {
      let arr = chartRow.appArray;
      arr[0] = arr[0] + 1;
...
router.post("/application", async (req, res, next) => {
  ...
  const jobTitle = await Job.findOne({_id:new ObjectId(req.body.jobId) },{"_id":0,"jobTitle":1});
...
// new Chart less application
LessChart.findOne({ jobTitle:jobTitle.jobTitle}, function (err, chartRow) {
    console.log(err);
    if (chartRow) {
      let arr = chartRow.appArray;
      arr[0] = arr[0] + 1;
